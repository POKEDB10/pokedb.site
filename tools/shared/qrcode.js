/**
 * Pure JavaScript QR Code Generator Library — pokedb.site
 * Zero external dependencies. Renders SVG, Canvas, and Base64 Data URLs.
 */
(function (root, factory) {
  ...
  for (var i = 8; i < 256; i++) QRMath.EXP_TABLE[i] = QRMath.EXP_TABLE[i - 1] ^ QRMath.EXP_TABLE[i - 2] ^ QRMath.EXP_TABLE[i - 4] ^ QRMath.EXP_TABLE[i - 8];
  ...
  QRCodeModel.prototype = {
    addData: function (data) {
      var newItem = new QR8bitByte(data);
      ...
      if (this.typeNumber < 1) {
        ...
        for (typeNumber = 1; typeNumber < 40; typeNumber++) {
          ...
          for (var i = 0; i < this.dataList.length; i++) {
            ...
            buffer.put(item.getLength(), QRUtil.getLengthInBits(item.mode, typeNumber));
            item.write(buffer);
          }
  ...
  QR8bitByte.prototype = {
    getLength: function () {
      var encodeURI = encodeURIComponent(this.data);
      var length = 0;
      for (var i = 0; i < encodeURI.length; i++) {
        if (encodeURI.charAt(i) === '%') { length++; i += 2; } else { length++; }
      }
      return length;
    },
    write: function (buffer) {
      var encodeURI = encodeURIComponent(this.data);
      for (var i = 0; i < encodeURI.length; i++) {
        var c = encodeURI.charAt(i);
        if (c === '%') {
          buffer.put(parseInt(encodeURI.substring(i + 1, i + 3), 16), 8);
          i += 2;
        } else {
          buffer.put(c.charCodeAt(0), 8);
        }
      }
    }
  };
  ...
  var QRCodeLibApi = {
    ...
    generateSVG: function (text, options) {
      ...
      var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + totalSize + ' ' + totalSize + '" width="100%" height="100%" shape-rendering="crispEdges">\n';
      ...
      if (icon) {
        var centerSize = Math.floor(size * 0.25);
        var cx = margin + Math.floor((size - centerSize) / 2);
        var cy = margin + Math.floor((size - centerSize) / 2);
        svg += '  <rect x="' + cx + '" y="' + cy + '" width="' + centerSize + '" height="' + centerSize + '" rx="1" fill="' + bg + '" stroke="' + fg + '" stroke-width="0.3"/>\n';
        svg += '  <text x="' + (cx + centerSize / 2) + '" y="' + (cy + centerSize / 2 + 0.8) + '" font-family="monospace" font-weight="bold" font-size="' + (centerSize * 0.6) + '" fill="' + fg + '" text-anchor="middle" dominant-baseline="middle">' + icon + '</text>\n';
      }
    ...
    generateDataURL: function (text, options) {
      options = options || {};
      var svg = this.generateSVG(text, options);
